...
def create_cdt_code_system(dts)
...
def create_cdt_structure_definitions(dts)
    ...
    dt.components.each do |comp|
      ...
      unless opt == 'W'
        el[:min]   =  case opt
                      when 'R'
                        1
                      when 'RE' # data absent reason is allowed in FHIR.  I don't know what the hell this means in v2
                        1
                      when 'O', 'C'
                        0
                      when 'B' # FIXME is this correct???
                        0
                      # when 'W'
                      #   nil
                      else
